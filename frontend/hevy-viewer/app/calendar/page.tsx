...
import { fetchAllRoutineAnalytics, fetchRoutines, fetchWorkouts } from "../exercises/api";
import { RoutineAnalytics, RoutineComparisonPoint, RoutineSummary, WorkoutSummary } from "../exercises/types";
...
function DayDot({ date, sessions, selected, isToday, todayKey, onSelect }: { date: Date; sessions: Session[]; selected: boolean; isToday: boolean; todayKey: string; onSelect: (key: string) => void }) {
  ...
  return (
    <button
      ...
      title={`${title} · ${sessions.map((session) => (session.routine ? `${session.title} ${pct(session.point.change_vs_previous_pct, 0)}` : session.title)).join(", ")}`}
      className={`aspect-square rounded-[3px] ${ring} ${selected ? "outline outline-2 outline-offset-1 outline-[var(--foreground)]" : "hover:outline hover:outline-1 hover:outline-offset-1 hover:outline-[var(--muted)]"}`}
      style={{ background: `color-mix(in oklch, ${sessions[0].routine ? toneVar(change) : "var(--muted)"} 78%, var(--background))` }}
    />
  );
}
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      ...
      try {
        const [routineList, allAnalytics, workoutList] = await Promise.all([fetchRoutines(credentials), fetchAllRoutineAnalytics(credentials), fetchWorkouts(credentials)]);
                ...
                <div className="flex flex-col gap-1.5 pt-3">
                  {weeks.map((week) => (
                    <div key={dayKey(week.start)} className="grid items-center gap-1.5" style={ROW_COLUMNS}>
                      ...
                      {week.days.map((date) => {
                        const key = dayKey(date);
                        return <DayDot key={key} date={date} sessions={byDay.get(key) ?? []} selected={key === activeKey} isToday={key === todayKey} todayKey={todayKey} onSelect={setSelectedKey} />;
                      })}
                    </div>
                  ))}
                </div>
